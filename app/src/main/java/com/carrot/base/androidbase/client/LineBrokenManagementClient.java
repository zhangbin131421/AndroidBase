 ...
 */
@Rest(rootUrl = "http://119.90.140.156:8087/api/LineBrokenManagement",
...
public interface LineBrokenManagementClient extends RestClientErrorHandling {
    ...
    LineBrokenManagementResult getById(@Path int id);

    @Get("/GetByUserID/?UserID={userId}")
    @Accept(MediaType.APPLICATION_JSON)
    List<LineBrokenManagementResult> getByUserId(@Path int userId);


    @Get("/GetByUserIDAndHandled/?UserID={userId}&IsHandled={isHandled}")
    @Accept(MediaType.APPLICATION_JSON)
    List<LineBrokenManagementResult> getByUserId(@Path int userId, @Path int isHandled);


    @Post("/AddNew")
    @Accept(MediaType.APPLICATION_JSON)
    void add(@Body LineBrokenManagementResult equipmentCheckResult);
