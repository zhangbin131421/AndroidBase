...
import com.carrot.base.androidbase.adapter.TaskCardOrderHandleAdapter;
...
public class TaskListOrderHandleActivity extends TaskListBaseActivity {

    @RestService
    OrderHandleClient orderHandleClient;

    private TaskCardOrderHandleAdapter mAdapter;
    ...
    void newAdapter(){
        mAdapter = new TaskCardOrderHandleAdapter(new ArrayList<OrderHandleResult>());
    }
    ...
    void uiInit(){
        ...
        ((TaskCardOrderHandleAdapter) mAdapter).setOnItemClickListener(new TaskCardOrderHandleAdapter
                .MyClickListener() {
            @Override
    ...
    List<OrderHandleResult> getResultListFromServer(){
        return orderHandleClient.getByUserId(userPrefs.id().get(), status.equals("已完成") ? 1 : 0);
    }
