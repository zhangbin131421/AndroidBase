...
import com.carrot.base.androidbase.adapter.TaskCardCoreMeterTestAdapter;
...
public class TaskListCoreMeterTestActivity extends TaskListBaseActivity {

    @RestService
    CoreMeterTestClient coreMeterTestClient;

    private TaskCardCoreMeterTestAdapter mAdapter;
    ...
    void newAdapter(){
        mAdapter = new TaskCardCoreMeterTestAdapter(new ArrayList<CoreMeterTestResult>());
    }
    ...
    void uiInit(){
        ...
        ((TaskCardCoreMeterTestAdapter) mAdapter).setOnItemClickListener(new TaskCardCoreMeterTestAdapter
                .MyClickListener() {
            @Override
    ...
    List<CoreMeterTestResult> getResultListFromServer(){
        return coreMeterTestClient.getByUserId(userPrefs.id().get(), status.equals("已完成") ? 1 : 0);
    }
