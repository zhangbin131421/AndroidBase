 ...
 */
@Rest(rootUrl = "http://119.90.140.156:8087/api/TotalPerformanceTest",
        converters = {MappingJackson2HttpMessageConverter.class,
                StringHttpMessageConverter.class,FormHttpMessageConverter.class,
                ByteArrayHttpMessageConverter.class})
public interface TotalPerformanceTestClient extends RestClientErrorHandling {

    @Get("/GetByID/?ID={id}")
    @Accept(MediaType.APPLICATION_JSON)
    TotalPerformanceTestResult getById(@Path int id);

    @Get("/GetByUserID/?UserID={userId}")
    @Accept(MediaType.APPLICATION_JSON)
    List<TotalPerformanceTestResult> getByUserId(@Path int userId);
    ...
    @Post("/AddNew")
    @Accept(MediaType.APPLICATION_JSON)
    void add(@Body TotalPerformanceTestResult equipmentCheckResult);
